...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32, return_square_img=False):
    ...
    if return_square_img:
        diff = abs(new_unpad[0] - new_unpad[1] + dw - dh)
        if new_unpad[0] > new_unpad[1]:
            dh += diff
        else:
            dw += diff
